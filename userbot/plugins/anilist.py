...
async def anime_doqnload(event):
    ...
    if input_str == "kaizoku":
        ...
        if search_result:
            result = f"<a href={search_url}>Clique aqui para obter mais resultados</a> <b>of</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKaizoku</code>: \n\n"
        ...
        result = f"<a href={search_url}>Clique aqui para obter mais resultados</a> <b>of</b> <code>{html.escape(search_query)}</code> <b>on</b> <code>AnimeKayo</code>: \n\n"
        for entry in search_result:
