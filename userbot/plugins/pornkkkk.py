...
@catub.cat_cmd(
    ...
    info={
        "header": "Pega uma foto ou gif de pornô.",
        "usage": [
            "{tr}porn",
...
async def very(event):
    ...
    await edit_or_reply(event, "**Espere um segundo garoto com tesão...😏**")
    ...
    while max_try < 5:
        ...
        try:
            ...
            return await edit_delete(
                event,
                "**(ノಠ益ಠ)ノ  Você tem certeza de que é uma categoria/subreddit válido??**",
                time=20,
            )
        ...
        try:
            ...
            await edit_or_reply(event, f"**Erro de valor!!..Link é :** {media_url}")
            await asyncio.sleep(3)
            await edit_or_reply(
                event,
                f"**Apenas segure sua vela e espere ...\n\nLimite de tentativas automáticas = {max_try+1}/5**",
            )
            ...
            if max_try == 5:
                await edit_delete(
                    event,
                    "**ಥ‿ಥ   Desculpe, não consegui encontrar, tente com outra categoria**",
                )
...
@catub.cat_cmd(
    ...
    info={
        "header": "baixe vídeos pornôs, gifs ou fotos em massa.",
        "usage": [
            "{tr}bulkporn",
...
async def bad(event):
    ...
    count = int(count)
    if count > 30:
        return await edit_delete(event, "**Erro de valor!.. Valor de contagem 1 a 30**")
    await edit_or_reply(event, "**Espere um segundo garoto com tesão...😏**")
    if await age_verification(event, reply_to):
        return
    flag = await useless.importent(event)
    ...
    try:
        ...
        return await edit_delete(event, "Erro de valor!.")
    title = []
    postlink = []
    media_url = []
    try:
        ...
        return await edit_delete(
            event, "**(ノಠ益ಠ)ノ  Tem certeza de que é uma categoria/subreddit válido??**", time=20
        )
    i = 0
    for m, p, t in zip(media_url, postlink, title):
        ...
        try:
            ...
            await edit_or_reply(
                event,
                f"**Download do Bluk iniciado.\n\nCatagoria :  `{sub_r}`\nArquivo baixando:  {i+1}/{count}**",
            )
            await asyncio.sleep(2)
        except WebpageCurlFailedError:
            await event.client.send_message(
                event.chat_id, f"**Erro de valor!!..Link é :** {m}"
            )
        i += 1
        if i == count:
            await event.delete()
...
async def cat(event):
    ...
    await edit_or_reply(event, "**Espere um minuto, garoto com tesão...😏**")
    listlink = []
    listname = []
    for i in col:
        a = i.find("a")
